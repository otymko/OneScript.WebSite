...
Функция ПолучитьСоставСборки(ИмяСборки) Экспорт
	...
	Если ФС.КаталогСуществует(ПутьККаталогу) Тогда
		...
		ДобавитьДистрибутивы(ПутьККаталогу, ИмяСборки, Архитектуры.Арх86);
		ДобавитьДистрибутивы(ПутьККаталогу, ИмяСборки, Архитектуры.Арх64);
		ДобавитьДистрибутивы(ПутьККаталогу, ИмяСборки, Архитектуры.Универсальная);
...
Процедура ДобавитьДистрибутивы(Знач БазовыйКаталог, Знач Версия, Знач Архитектура)

	Подкаталог = ОбъединитьПути(БазовыйКаталог, Архитектуры.ПодкаталогАрхитектуры(Архитектура));
		...
		Для Каждого ВариантСборки Из Сборка.СписокВаринтовСборки Цикл
			...
			Если ФайлВариантаСборки <> Неопределено Тогда
				Сборка.ДобавитьСтрокуСоставаСборки(
					...
					Сборка.ПолучитьСсылкуНаФайл(ОбъединитьПути(Версия, Подкаталог), ФайлВариантаСборки.Имя), 
