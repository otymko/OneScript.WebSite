...
Функция ПреобразоватьФорматДаты(Знач ВходящееЗначение) Экспорт
	
	Значение = ВходящееЗначение;
	Массив = СтрРазделить(ВходящееЗначение, "-");
	Если Массив.Количество() = 3 Тогда
		Значение = СтрШаблон("%1.%2.%3", Массив[2], Массив[1], Массив[0]);
	КонецЕсли;
...
Функция ПолучитьБитностьИзКонтроллера(Контроллер) Экспорт
	
	Значение = Контроллер.ЗапросHttp.ПараметрыЗапроса()["bitness"];
	Если Не ЗначениеЗаполнено(Значение) Тогда
		Значение = Контроллер.ЗначенияМаршрута.Получить("bitness");
	КонецЕсли;
