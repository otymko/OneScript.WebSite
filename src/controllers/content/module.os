...
Функция Index() Экспорт
	...
	ЛокальныеПараметры = Новый ЛокальныеПараметрыСтраницы(ПараметрыСтраницы.НавигационноеМеню.ЗаголовокРаздела, ЭтоРаздел(), "/" + Раздел);
	ПараметрыСтраницы.ЛокальнаяНавигация.Добавить(ЛокальныеПараметры);
	
	Если ЗначениеЗаполнено(Подраздел) Тогда
		ЛокальныеПараметры = Новый ЛокальныеПараметрыСтраницы(ПараметрыСтраницы.НавигационноеМеню.ЗаголовокПодраздела, ЭтоПодраздел(), "/" + ОбъединитьПути(Раздел, Подраздел));
		ПараметрыСтраницы.ЛокальнаяНавигация.Добавить(ЛокальныеПараметры);
	КонецЕсли;

	Если ЗначениеЗаполнено(Страница) Тогда
		ЛокальныеПараметры = Новый ЛокальныеПараметрыСтраницы(ПараметрыСтраницы.НавигационноеМеню.ЗаголовокСтраницы, ЭтоСтраница());
		ПараметрыСтраницы.ЛокальнаяНавигация.Добавить(ЛокальныеПараметры);
	КонецЕсли;

	Если Не ЗначениеЗаполнено(ПараметрыСтраницы.Заголовок) Тогда
		ПараметрыСтраницы.Заголовок = "1Script - " + ПараметрыСтраницы.НавигационноеМеню.ЗаголовокРаздела;
	КонецЕсли;
