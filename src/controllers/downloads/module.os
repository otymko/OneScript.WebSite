...
Функция Index() Экспорт
	...
	ПараметрыПредставления.Вставить("Манифест", ОтобразитьМанифест());
	ПараметрыПредставления.Вставить("АктуальнаяСборка", УправлениеКонтентом.ПолучитьСоставСборки("latest"));
	ПараметрыПредставления.Вставить("СтабильнаяВерсия", УправлениеКонтентом.ПолучитьСоставСборки("lts"));
	ПараметрыПредставления.Вставить("ОзнакомительнаяВерсия", УправлениеКонтентом.ПолучитьСоставСборки("preview"));
	//ПараметрыПредставления.Вставить("НочнаяСборкаАктуальной", УправлениеКонтентом.ПолучитьСоставСборки("latest-dev"));
...
Функция Getfiledata() Экспорт
	...
	Если ДистрибутивыВерсии.Существует() Тогда
		...
		Если ВариантДистрибутива <> Неопределено Тогда
			Возврат Файл(
				ВариантДистрибутива.Файл.ПолноеИмя,
				"application/octet-stream",
				ДистрибутивыВерсии.ТокенВерсии() + ВариантДистрибутива.Файл.Расширение);
		КонецЕсли;
	КонецЕсли;
...
Функция ОтобразитьМанифест()
	Модель = Новый ТаблицаЗначений();
	Модель.Колонки.Добавить("Идентификатор");
	Модель.Колонки.Добавить("Описание");
	Для Каждого Вариант Из УправлениеКонтентом.ПолучитьМанифест().ПолучитьДистрибутивы() Цикл
		Стр = Модель.Добавить();
		Стр.Идентификатор = Вариант.ПолучитьИдентификатор();
		Стр.Описание = Вариант.ПолучитьВидДистрибутива();
	КонецЦикла;
	Возврат Модель;
КонецФункции
